...
const getChart = (node, data = [], options = {}) => {
    let width = options?.width ?? defaultOptions.width,
        ...
        xAxisOptions = {
            ...
            visibility:
                options?.xAxis?.visibility ?? defaultOptions.xAxis.visibility,
        ...
        yAxisOptions = {
            ...
            visibility:
                options?.yAxis?.visibility ?? defaultOptions.yAxis.visibility
        },
        barWidth = options?.barWidth ?? defaultOptions.barWidth,
        barTopRadius = options?.barTopRadius ?? defaultOptions.barTopRadius,
        barBottomRadius =
            options?.barBottomRadius ?? defaultOptions.barBottomRadius,
        xAxisTextHeight =
            options?.xAxisTextHeight ?? defaultOptions.xAxisTextHeight,
        yAxisTextWidth =
            options?.yAxisTextWidth ?? defaultOptions.yAxisTextWidth,
        dataLabels = {
            enabled:
                options?.dataLabels?.enabled ??
                defaultOptions.dataLabels.enabled,
            fill: options?.dataLabels?.fill ?? defaultOptions.dataLabels.fill,
            fontSize:
                options?.dataLabels?.fontSize ??
                defaultOptions.dataLabels.fontSize,
            height:
                options?.dataLabels?.fontSize ??
                defaultOptions.dataLabels.height
        },
        dataLabelsWrapper = {
            enabled:
                options?.dataLabelsWrapper?.enabled ??
                defaultOptions.dataLabelsWrapper.enabled,
            fill:
                options?.dataLabelsWrapper?.fill ??
                defaultOptions.dataLabelsWrapper.fill,
            fontSize:
                options?.dataLabelsWrapper?.fontSize ??
                defaultOptions.dataLabelsWrapper.fontSize,
            radius:
                options?.dataLabelsWrapper?.radius ??
                defaultOptions.dataLabelsWrapper.radius,
            showArrow:
                options?.dataLabelsWrapper?.showArrow ??
                defaultOptions.dataLabelsWrapper.showArrow,
            arrowSize:
                options?.dataLabelsWrapper?.arrowSize ??
                defaultOptions.dataLabelsWrapper.arrowSize,
            padding: {
                top:
                    options?.dataLabelsWrapper?.padding?.top ??
                    defaultOptions.dataLabelsWrapper.padding.top,
                left:
                    options?.dataLabelsWrapper?.padding?.left ??
                    defaultOptions.dataLabelsWrapper.padding.left,
                right:
                    options?.dataLabelsWrapper?.padding?.right ??
                    defaultOptions.dataLabelsWrapper.padding.right,
                bottom:
                    options?.dataLabelsWrapper?.padding?.bottom ??
                    defaultOptions.dataLabelsWrapper.padding.bottom
            }
        };
    ...
    var yScale = d3
        ...
        .range([
            data && data.length > 0
                ? height -
                  xAxisTextHeight -
                  (dataLabels.enabled ? dataLabels.height : 0)
                : 0,
            0
        ])
        .domain([
    ...
    if (xAxisOptions.visibility === true) {
        ...
        xAxisGroup
            ...
            .attr(
                "transform",
                "translate(0," + (height - xAxisTextHeight) + ")"
            )
            .attr("class", "xAxis")
            .merge(xAxisGroup)
            .transition()
        ...
        if (xAxisOptions.lines === false) {
            parentGroup
                .selectAll("g.xAxis .tick line")
                .style("display", "none");
        }
    }

    if (yAxisOptions.visibility === true) {
        var yAxisGroup = parentGroup
            .selectAll(".yAxis")
            .data(data?.length > 0 ? [0] : []);
    ...
    bars.enter()
        ...
        .attr("x", function (d) {
            return (
                xScale(d.key) +
                (barWidth && barWidth > 0
                    ? (xScale.bandwidth() + barWidth) / 2 - barWidth
                    : 0)
            );
        })
        .attr("y", height - xAxisTextHeight)
        ...
        .on("mousemove", function (event, d) {
            Tooltip(event, d, { className: "verticalTooltip" });
        })
        .on("mouseout", Tooltip.hide)
        .attr("width", function (d) {
            return barWidth && barWidth > 0 ? barWidth : xScale.bandwidth();
        })
        .transition()
        .duration(1000)
        .attr("fill", function (d) {
        ...
        .attr("y", function (d) {
            return (
                yScale(d.value) + (dataLabels.enabled ? dataLabels.height : 0)
            );
        })
    ...
    let textWrapperGroup = parentGroup
        .selectAll(".textWrapperGroup")
        .data(dataLabels.enabled ? data : []);
        ...
        .each(function (eachData) {
            ...
            if (dataLabels.enabled) {
                text.enter()
                    ...
                    .style("opacity", 0)
                    .text(function (d) {
                        return d?.displayValue?.value ?? d?.displayValue ?? "";
                    })
                    .merge(text)
            ...
            let textWrapper = group
                .selectAll(".verticalBarTextWrapper")
                .data([text]);
            textWrapper
                .exit()
                .transition()
            ...
            let wrapperArrow = group
                .selectAll(".wrapperArrow")
                .data([eachData]);
            wrapperArrow
                .exit()
                .transition()
            ...
            if (dataLabelsWrapper.enabled) {
                var triangle = d3
                    ...
                    .size([dataLabelsWrapper.arrowSize]);
                textWrapper
                    .enter()
                    ...
                    .attr("width", function (d) {
                        return (
                            ...
                            dataLabelsWrapper.padding.left +
                            dataLabelsWrapper.padding.right
                        );
                    })
                    .attr("height", function (d) {
                        return (
                            d.node().getBBox().height +
                            dataLabelsWrapper.padding.top +
                            dataLabelsWrapper.padding.bottom
                        );
                    })
                    .attr("fill", dataLabelsWrapper.fill)
                    .attr("x", function (d) {
                        return (
                            -(d.node().getBBox().width / 2) -
                            dataLabelsWrapper.padding.left
                        );
                    })
                    .transition()
                ...
                wrapperArrow
                    ...
                    .attr("transform", function (d) {
                        return (
                            "translate(0, " +
                            (dataLabelsWrapper.arrowSize / 4 - 0.5) +
                            ") rotate(180)"
                        );
                    });
            }
